...
class RotinaBD():
    ...
    '''Funcao principal que recebe os dados da pergunta e retorna possiveis respostas'''
    '''entrada = dados {'entidade': [], propriedade:[], indPergunta:[], sinonimosPropriedade:[[]]}'''
    '''saida = Lista com possiveis respostas dispostas como [Entidade, Propriedade, [Valor]]'''
    def buscar(self, dados):
        ...
        if (not self.verificarEntidade(consulta['entidade'])):
    ...
    def buscarInformacao(self, dados):
        resposta = []
        # buscar o valor na relação (entidade, propriedade, valor)
        try:
            ...
            raise e
            print(e)
        # Verificar se o valor é um id de entidade e, se sim, buscar o nome dessa entidade
        if resposta != []:
            nomeEntidades = []
            for valor in resposta:
                p = re.compile('Q\d+')
                if p.match(valor) != None:
                    nomeEntidades.append(self.baixarNomeEntidade(valor))
            if nomeEntidades != []:
                resposta = nomeEntidades
        return resposta

################################################################ Funções para uso da api do Wikidata ######################################### 
    '''
...
################################################ Funções para tratar os dados extraídos da api da WIKIDATA ####################################### 
